...
/// <summary>
/// Tests for the <see cref="Flux.ReconcileAsync(FluxResource, string, string, CancellationToken)"/> method.
/// </summary>
[Collection("Flux")]
public class ReconcileAsyncTests
{
  ...
  {
    // Act
    var exception = await Record.ExceptionAsync(async () => await Flux.ReconcileAsync(FluxResource.Source, "test").ConfigureAwait(false));
